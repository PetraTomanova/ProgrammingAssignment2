...
## functions do
  ...
  # I wrote two functions that are used to create a special object 
...
makeCacheMatrix <- function(x = matrix()) {
  ...
  list(set = set, get = get,  # list containing the functions
...
cacheSolve <- function(x, ...) {
  ...
  if(!is.null(m)) {  # check if the inverse has already been calculated
    ...
    return(m)  # get the inverse from the cache and skips the computation
  }
  data <- x$get()
  m <- solve(data, ...)
  x$setinverse(m)
